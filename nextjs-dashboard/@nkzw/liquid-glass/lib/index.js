...
const GlassContainer = forwardRef(({ aberrationIntensity = 2, blurAmount = 12, borderRadius = 999, children, className = "", height, width, customSize, displacementScale = 25, glassSize, mode = "standard", onClick, onMouseDown, onMouseEnter, onMouseLeave, onMouseUp, overLight = false, padding = "24px 32px", saturation = 180, style }, ref) => {
...
function LiquidGlass({ aberrationIntensity = 2, blurAmount = .0625, borderRadius = 999, children, height, width, customSize, className = "", displacementScale = 70, elasticity = .15, globalMousePos: externalGlobalMousePos, mode = "standard", mouseContainer = null, mouseOffset: externalMouseOffset, onClick, overLight = false, padding = "24px 32px", saturation = 140, style }) {
	...
	useLayoutEffect(() => {
		const updateGlassSize = () => {
			if (glassRef.current) {
				...
				if (customSize && glassRef.current.parentNode.parentNode) {
					const { offsetHeight, offsetWidth } = glassRef.current.parentNode.parentNode;
					console.log("offsetHeight", offsetHeight, "offsetWidth", offsetWidth);
					let offsetHeightCustom = null;
					let offsetWidthCustom = null
					offsetWidthCustom = offsetWidth;
					offsetHeightCustom = offsetHeight;

					if (height && width) {
						let tempheight = offsetHeight * height;
						let tempWidth = offsetWidth * width;
						tempheight = Math.round(tempheight);
						tempWidth = Math.round(tempWidth);
						offsetHeightCustom = tempheight;
						offsetWidthCustom = tempWidth;
					}
					console.log("offsetHeightCustom", offsetHeightCustom, "offsetWidthCustom", offsetWidthCustom);
					
					setGlassSize({
					height: offsetHeightCustom,
				...
				let offsetHeightCustom = offsetHeight;
				let offsetWidthCustom = offsetWidth;
